...
import at.ait.dme.yuma.suite.client.server.FindPlaceResponse;
import at.ait.dme.yuma.suite.client.server.FindPlaceService;
import at.ait.dme.yuma.suite.client.server.FindPlaceServiceAsync;
...
public class SearchPanel extends FlowPanel {
	...
	private void doAsyncGeocoding() {
		...
		if (searchBox.getValue().trim().length() > 0) {
			service.findPlace(Application.getImageUrl(), searchBox.getValue(), new AsyncCallback<FindPlaceResponse>() {
				@Override
