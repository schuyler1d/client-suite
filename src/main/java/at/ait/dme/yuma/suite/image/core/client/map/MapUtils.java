...
package at.ait.dme.yuma.suite.image.core.client.map;
...
import at.ait.dme.yuma.suite.image.core.client.shape.Polygon;
import at.ait.dme.yuma.suite.image.core.client.shape.Polyline;
...
public class MapUtils {
	...
	public static VectorFeature shapeToVectorFeature(Shape shape) {
		if (shape instanceof Polyline) {
			...
			for (at.ait.dme.yuma.suite.image.core.client.shape.Point p :  ((Polyline) shape).getPoints()) {
			...
			// Polygon
			ArrayList<Point> points = new ArrayList<Point>();			
			for (at.ait.dme.yuma.suite.image.core.client.shape.Point p :  ((Polygon) shape).getPoints()) {
				points.add(Point.create(p.getX() + shape.getLeft(), p.getY() + shape.getTop()));
			}
	...
	public static Shape vectorFeatureToShape(VectorFeature feature) {
		...
		if (js.startsWith("POLYGON")) {
			...
			ArrayList<at.ait.dme.yuma.suite.image.core.client.shape.Point> points = 
				new ArrayList<at.ait.dme.yuma.suite.image.core.client.shape.Point>();
			...
			for (int i=0; i<s.length; i++) {
				...
				if (xy.length == 2) {
					try {
						...
						points.add(new at.ait.dme.yuma.suite.image.core.client.shape.Point((int) x, (int) y));
			...
			poly.setLeft(left);
			poly.setTop(top);
			for (at.ait.dme.yuma.suite.image.core.client.shape.Point p : points) {	
				poly.addPoint(new at.ait.dme.yuma.suite.image.core.client.shape.Point(p.getX() - left,p.getY() - top));
			}
			...
			for (int i=0; i<points.length; i++) {
				...
				if (xy.length == 2) {
					try {
						double x = Double.parseDouble(xy[0]);
						double y = Double.parseDouble(xy[1]);
						pline.addPoint(new at.ait.dme.yuma.suite.image.core.client.shape.Point((int)x,(int)y));
					} catch (NumberFormatException e) {
						System.out.println("Error adding point: " + e.getMessage());
					}
				}
