...
import at.ait.dme.yuma.suite.client.image.shape.Line;
	...
	private static String createLinkedToResourcesFragment(ImageAnnotation annotation) throws IOException {
        ...
        if(annotation.hasSemanticTags()) {
		    for(SemanticTag t : annotation.getSemanticTags()) {
		        ...
		        if (t.hasAltLabels()) {
		        	List<String> altLabels = t.getAlternativeLabels();
		        	StringBuffer sb = new StringBuffer();
		        	for (int i=0; i<altLabels.size(); i++) {
		        		sb.append(altLabels.get(i) + ";");
		        	}
		        	Element altLabelsEl = new Element("altLabels", ANNOTATION_REL_NAMESPACE);
		        	altLabelsEl.setText(sb.toString());
		        	tagEl.addContent(altLabelsEl);
		        }
        ...
        try {
            ...
            for(Element e : resources) {
                if(e.getName().equals("linked-resource")) {
                    ...
                    String allAltLabels = StringEscapeUtils.unescapeXml(e.getChildText("altLabels", ANNOTATION_REL_NAMESPACE));
                    if (allAltLabels != null) {
                    	String[] array = allAltLabels.split(";");
                    	for (int i=0; i<array.length; i++) {
                    		altLabels.add(array[i]);
                    	}
                    }
                    semanticTags.add(new SemanticTag(title, altLabels, type, language, description, seeAlso));
                }
            }
